# ===================== SHAP ANALYSIS FOR PI-TRANSFORMER (with units) =====================
import torch
import numpy as np
import pandas as pd
...
model_path        = "/home/admin/BurningGH2/PI_TRANSFORMER_WINDPOWER_IDEALGAS/PI_Transformer_model_idealgas.h5"
save_dir          = "/home/admin/BurningGH2/SHAP_ANALYSIS_PI_TRANSFORMER_WINDPOWER_IDEALGAS"
os.makedirs(save_dir, exist_ok=True)

# ============ LOGGING ============
log_file_path = os.path.join(save_dir, "SHAP_ANALYSIS_PI_TRANSFORMER_WINDPOWER_IDEALGAS.log")
sys.stdout = open(log_file_path, 'w')
sys.stderr = sys.stdout
print = functools.partial(print, flush=True)

print("[INFO] SHAP analysis (with units) starting...")
...
# --------- Unit map (from your verification log) ----------
...
unit_map = {
    ...
    "Ambient temperature (Hourly, SCADA)": f"Ambient temperature ({degree}C)",
    "temperature_K": "Temperature (K)",
    "Rotor Speed (Hourly, SCADA)": "Rotor Speed (rpm)",
    "rho_kgm-3": "Air density (kg/m³)",
    "Wind Speed (Hourly, SCADA)": "Wind Speed (SCADA) (m/s)",
}
pretty_names = [unit_map.get(n, n) for n in feature_names]
...
class TransformerModel(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        power_output = torch.relu(output[:, :, 0])   # SAME activations as your NB2
...
background_data = X_train_tensor[:100].mean(dim=1)   # background (mean over sequence)
test_samples    = X_test_tensor[:200].mean(dim=1)    # samples to explain (mean over sequence)
...
# ============ Aggregate and Plot (Bar chart) ============
shap_values     = np.mean(np.abs(all_shap_values), axis=0)  # [samples, features]
mean_shap_vals  = shap_values.mean(axis=0)                  # [features]
raw_power       = train_df[target_cols[0]].values
power_range     = raw_power.max() - raw_power.min()
adjusted_shap_vals = mean_shap_vals * power_range           # put in power units

sorted_idx  = np.argsort(adjusted_shap_vals)[::-1]
top_n       = min(20, len(feature_names))
top_idx     = sorted_idx[:top_n]
top_vals    = adjusted_shap_vals[top_idx]
top_labels  = np.array(pretty_names)[top_idx]

plt.figure(figsize=(10, 6 + 0.3 * top_n))
y_pos = range(len(top_vals))[::-1]
plt.barh(y_pos, top_vals, color='crimson')
...
plt.xlabel("PI_Transformer SHAP value (impact on model output)", fontsize=12)  # <- your requested label
plt.title("Top Features by SHAP Value (Power Forecast)", fontsize=14, pad=15)
plt.grid(axis='x', linestyle='--', alpha=0.4)
plt.tight_layout()
bar_path = os.path.join(save_dir, "PI_Transformer_SHAP_power_mean_all_horizons.png")
plt.savefig(bar_path, dpi=300, bbox_inches='tight')
plt.close()
print(f"[INFO] SHAP bar plot saved: {bar_path}")

# Save SHAP values to CSV (with pretty names)
shap_df = pd.DataFrame({'feature': pretty_names, 'mean_shap_value_power_units': adjusted_shap_vals})
shap_df = shap_df.sort_values('mean_shap_value_power_units', ascending=False)
csv_path = os.path.join(save_dir, "shap_values.csv")
shap_df.to_csv(csv_path, index=False)
print(f"[INFO] SHAP values saved to CSV: {csv_path}")

# ============ Beeswarm for top 20 (with unit labels) ============
print("[INFO] Generating SHAP beeswarm (top 20 features) ...")
...
top_feature_names = [pretty_names[i] for i in sorted_idx]
...
fig = plt.gcf()
fig.axes[0].set_xlabel("PI_Transformer SHAP value (impact on model output)")
plt.tight_layout()
beeswarm_path = os.path.join(save_dir, "PI_Transformer_SHAP_beeswarm_top20_features.png")
plt.savefig(beeswarm_path, dpi=300, bbox_inches="tight")
plt.close()
print(f"[INFO] SHAP beeswarm plot saved: {beeswarm_path}")

print("[DONE] SHAP analysis finished.")
